function signal=MPRAGEfunc_varyingTR(nimages,MPRAGE_tr,inversiontimes,nZslices,FLASH_tr,flipangle,sequence,T1s,varargin);
...
if normalsequence
    for k=1:nimages
    E_1(k)=exp(-FLASH_tr(k)./T1s);    %recovery between two excitaion
    TA(k)=nZslices*FLASH_tr(k);
    TA_bef(k)=nZ_bef*FLASH_tr(k);
    TA_aft(k)=nZ_aft*FLASH_tr(k);
    end;
    TD(1)=inversiontimes(1)-TA_bef(1);
    E_TD(1)=exp(-TD(1)./T1s);
    TD(nimages+1)=MPRAGE_tr-inversiontimes(nimages)-TA_aft(end);
    E_TD(nimages+1)=exp(-TD(nimages+1)./T1s);
    if nimages>1
        for k=2:(nimages)
            TD(k)=inversiontimes(k)-inversiontimes(k-1)-(TA_aft(k-1) + TA_bef(k));
            E_TD(k)=exp(-TD(k)./T1s);
        end;
    end;
    for k=1:(nimages)
        cosalfaE1(k)=cos(fliprad(k))*(E_1(k));
        oneminusE1(k)=1-E_1(k);
        sinalfa(k)=sin(fliprad(k));
    end;
    
end;
% I commented out the Water excitation because it is not really that
% usefull and would mean
% if waterexcitation
%     
...
for k=1:(nimages)
    ...
    MZsteadystatenumerator=MZsteadystatenumerator*(cosalfaE1(k))^nZslices...
        +M0.*(1-E_1(k)).*(1-(cosalfaE1(k))^nZslices)...
        ./(1-(cosalfaE1(k)));
