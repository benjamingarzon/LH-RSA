...
#SUBJECTS="lue1202 lue1203 lue1204 lue1205 lue1206 lue1207"
SUBJECTS="lue1203 lue1204 lue1205 lue1206 lue1207"
SUBJECTS="lue1105"
...
if [ `ls $HOMEDIR/data_BIDS/sub-$SUBJECT/*/anat/sub-${SUBJECT}_ses-?_MP2RAGE.nii.gz | wc -l` -eq $NSESSIONS ]; then 
   ...
   if [ "$PHASE" -eq "3" ] || [ "$PHASE" -eq "11" ]; then
     nice ./pipeline.sh $SUBJECT 1 $RESPONSES_FILE $NTRIALS $PHASE $RUNS "$RESPONSES_OPTIONS" "$CONFOUND_INDICES" $HOMEDIR > $HOMEDIR/logs/${SUBJECT}.phase${PHASE}.log
   else
