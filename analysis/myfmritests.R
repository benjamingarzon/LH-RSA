...
{ 
  var.names = c("INTERCEPT", "SYSTEM", "CONFIGURATION", "GROUP", "CONDITION", "TRAINING", 
                "TRAINING.Q", "GROUP_x_TRAINING", "GROUP_x_TRAINING.Q")
  contrast.names = c("GROUP_x_TRAINING+", "GROUP_x_TRAINING.Q+", "GROUP_x_TRAINING-", )
  ...
  X$y = y
  # SYSTEM
  model = lmer(y ~ 1 + SYSTEM + CONFIGURATION + GROUP*CONDITION*(TRAINING + TRAINING.Q) + 
                 + (1 |SUBJECT), data = X)
