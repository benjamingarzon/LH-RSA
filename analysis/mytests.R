...
modelcomparison_depth = function(y, X, WAVES = seq(5))
{
  ...
  tryCatch({
    model.l = lmer(
      y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * TRAINING + poly(TRAINING, deg, raw = T) +
        (1 | SUBJECT),
      data = X
    )
    
    model.a = lmer(
      y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * TRAINING.A + poly(TRAINING, deg, raw = T) +
        (1 | SUBJECT),
      data = X
    )
    
    model.q = lmer(
      y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * TRAINING.Q + poly(TRAINING, deg, raw = T) +
        (1 | SUBJECT),
      data = X
    )
    
    model.q2 = lmer(
      y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * poly(TRAINING, 2, raw = T) +  poly(TRAINING, deg, raw = T) +
        (1 | SUBJECT),
      data = X
    )
    
    model.q3 = lmer(
      y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * poly(TRAINING, deg, raw = T) +
        (1 | SUBJECT),
      data = X
    )
...
testasymptotic_depth = function(y, X, WAVES = seq(5))
{
  var.names = c(
    "INTERCEPT",
    "FD",
  ...
  model = lmer(
    y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * TRAINING.A + poly(TRAINING, 3, raw = T) +
      (1 | SUBJECT),
    data = X, contrasts=list(GROUP=contr.sum)
  )
...
testcubic_depth = function(y, X, WAVES = seq(5))
{
  var.names = c(
    "INTERCEPT",
    "FD",
  ...
  model = lmer(
    y ~ 1 + FD + SYSTEM + DEPTH + CONFIGURATION + GROUP * poly(TRAINING, 3, raw = T) +
      (1 | SUBJECT),
    data = X, contrasts=list(GROUP=contr.sum)
  )
...
modelcomparison_depth_old = function(y, X)
{
...
testquadratic_depth_old = function(y, X, ALTERNATIVE = 'greater')
{
...
testlinear_depth_old = function(y, X, ALTERNATIVE = 'greater')
{
