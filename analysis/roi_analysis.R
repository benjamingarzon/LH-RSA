...
radius = 5
DISTANCE=50
DATADIR = '~/Data/LeftHand/Lund1/cat12crossbias12_15' #8_10/'
...
myplots.T1.quadratic.groupxtraining = create_surf_rois(DATADIR, TESTDIR, TESTNAME, DISTANCE, radius, MASK_NAME, THR, 
                                                    myplots.thickness.linear.groupxtraining$ROI_FILE, wDEPTH = T)
TESTNAME = 'thickness_Omni_p'
myplots.T1.quadratic.omni = create_surf_rois(DATADIR, TESTDIR, TESTNAME, DISTANCE, radius, MASK_NAME, THR, 
                                                    myplots.thickness.quadratic.omni$ROI_FILE, wDEPTH = T)

TESTDIR = 'tests/T1/quadratic'
TESTNAME = 'GROUP_x_TRAINING_p'
myplots.T1.quadratic.groupxtraining = create_surf_rois(DATADIR, TESTDIR, TESTNAME, DISTANCE, radius, MASK_NAME, THR, wDEPTH = T)
...
model.l = lmer(y ~ 1 + SYSTEM + GROUP*TRAINING*DEPTH + (1|SUBJECT), data = X)
...
myplot = ggplot(X, aes(x = TP, group = SUBJECT, col = GROUP, y = y)) + geom_line() + geom_point() + facet_grid(. ~DEPTH) + 
...
myplot = ggplot(X.sem, aes(x = TP, group = GROUP, col = GROUP, y = y.mean, ymax = y.mean + y.sem, ymin = y.mean - y.sem)) + 
